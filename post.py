...
import feedparser
import random
from post_browser import post_to_threads

# ADHDに関連するRSSフィード
RSS_FEEDS = [
    ("nhk_health",   "https://www3.nhk.or.jp/rss/news/cat1.xml"),        # NHK健康・医療
    ("yomiuri",      "https://www.yomiuri.co.jp/feed/"),                   # 読売新聞
    ("asahi",        "https://www.asahi.com/rss/asahi/newsheadlines.rdf"), # 朝日新聞
    ("mainichi",     "https://mainichi.jp/rss/etc/mainichi-flash.rss"),    # 毎日新聞
]
...
ORIGINAL_TOPICS = [
    ...
    "Notionで作ったADHD向けタスク管理テンプレートの紹介",
    "衝動買いを防ぐために決めたルール",
    "ADHDの過集中を仕事に活かす方法",
]
...
def main():
    ...
    # ニュースがあれば50%の確率でニュース投稿、残りはオリジナル
    use_news = articles and random.random() < 0.5

    if use_news:
        article = random.choice(articles)
        print(f"使用記事: {article['title']}")
        post_text = generate_post_from_news(article)
        if article["url"] and random.random() < 0.5:
            full_text = f"{post_text}\n\n{article['url']}"
        else:
            full_text = post_text
    else:
        topic = random.choice(ORIGINAL_TOPICS)
        print(f"オリジナルトピック: {topic}")
    ...
    if result:
        save_post_log(article, full_text)
        print("投稿ログを保存しました")
